# AI Agent Workflow — whispr-messenger/scheduling-service
...
- GitHub org/repo: `whispr-messenger/scheduling-service`
...
Example: `WHISPR-498-consolidate-health-endpoints`
...
- Mock all I/O (database, cache, JWT service, S3/MinIO) — never hit real services.
...
Example:
```
fix(health): consolidate probe surface to /health
```
...
{ "projectKey": "whispr-messenger_scheduling-service" }
...
{
  "projects": ["whispr-messenger_scheduling-service"],
...
```bash
gh pr checks <PR-number> --repo whispr-messenger/scheduling-service
```
...
- CI green (`gh pr checks <PR-number> --repo whispr-messenger/scheduling-service`)
...
// → customfield_10020[0].id  (e.g. 200 for Sprint 6)
```
...
This project is indexed by GitNexus as **scheduling-service**. Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
3. `READ gitnexus://repo/scheduling-service/process/{processName}` — trace the full execution flow step by step
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/scheduling-service/context` | Codebase overview, check index freshness |
| `gitnexus://repo/scheduling-service/clusters` | All functional areas |
| `gitnexus://repo/scheduling-service/processes` | All execution flows |
| `gitnexus://repo/scheduling-service/process/{name}` | Step-by-step execution trace |
