...
describe('Entrypoint', () => {
	...
	describe('Successful startup', () => {
		it('should run environment checks and start the application', async () => {
			...
			expect(consoleLogSpy).toHaveBeenCalledWith('Starting Scheduling Service...\n');
	...
	describe('Failed startup', () => {
		...
		it('should not log "Starting Scheduling Service..." when checks fail', () => {
			// Arrange
			mockRunEnvChecks.mockImplementation(() => {
			...
			expect(consoleLogSpy).not.toHaveBeenCalledWith('Starting Scheduling Service...\n');
		});
	});
